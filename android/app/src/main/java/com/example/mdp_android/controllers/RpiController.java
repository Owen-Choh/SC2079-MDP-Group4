...
public class RpiController {
    private static final String TAG = "RpiController";
    ...
    public static String getRpiMessageType(String received) {
        if (received.contains("COORDINATES")) {
            return "robot";
        } else if (received.contains("image-rec")) {
            return "image";
    ...
    public static JSONObject readRpiMessages(String received) {
        try {
            ...
            if ("COORDINATES".equals(jsonObj.optString("cat"))) {
                // Get current coordinates of the robot
                return jsonObj;
            } else if ("image-rec".equals(jsonObj.optString("cat"))) {
                // Get image recognition results from RPi
                JSONObject results = jsonObj.optJSONObject("value");
                return results;
            } else if ("PATH".equals(jsonObj.optString("type"))) {
                // Get list of coordinates from RPi
                JSONObject path = jsonObj.optJSONObject("data");
                return path;
            }
    ...
    public static JSONObject getObstacleDetails(ArrayList<Map.Obstacle> obstacles) {
        ...
        try {
            ...
            Log.d(TAG, "Failed to parse obstacle string into json: ", e);
        }
        return message;
    }
    // get map details into json (including task type)
    public static JSONObject getStartMsg() {
        JSONObject message = new JSONObject();
        JSONObject data = new JSONObject();
        JSONArray obstaclesCoor = new JSONArray();
        try {
            ...
            Log.d(TAG, "Failed to parse start msg string into json: ", e);
        }
        return message;
    }

    // prev implementation of get map details into json (including task type)
