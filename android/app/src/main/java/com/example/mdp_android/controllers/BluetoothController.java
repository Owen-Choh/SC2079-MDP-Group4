    ...
    //            }
        ...
        private class ConnectedThread extends Thread {
            ...
            private StringBuilder messageBuffer; // Buffer to accumulate the message
            ...
            public ConnectedThread(BluetoothSocket socket) {
                ...
                messageBuffer = new StringBuilder(); // Initialize the message buffer
            }
            ...
            public void run() {
                ...
                while (mState == StateConstants.STATE_CONNECTED) {
                    try {
                        ...
                        // Convert the bytes to a string and append to the message buffer
                        String receivedChunk = new String(mmBuffer, 0, numBytes);
                        messageBuffer.append(receivedChunk);

                        // Check if the message is complete (e.g., ends with a newline or other delimiter)
                        if (receivedChunk.contains("\n")) {
                            // The complete message has been received
                            String completeMessage = messageBuffer.toString().trim(); // Trim to remove the delimiter
                            Log.d(TAG, "RECEIVED COMPLETE MSG: " + completeMessage);

                            // Send the complete message to the UI activity
                            Message readMsg = mHandler.obtainMessage(
                                    MessageConstants.MESSAGE_READ, completeMessage.length(), -1,
                                    completeMessage.getBytes());

                            // Add the received message to the repository
                            MessageRepository.getInstance().addReceivedMessage(getCurrentTime() + " " + completeMessage);
