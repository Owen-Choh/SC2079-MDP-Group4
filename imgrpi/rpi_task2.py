        ...
        while True:
            ...
            # control commands
                ...
                if message['value'] == "start":
                    self.stm_link.send("STARTaaa")  # Send "START" to STM32
                    self.logger.info("Sent 'START' command to STM32")

                                                            

    def recv_stm(self) -> None:
        self.logger.info("Listening for messages from STM32...")
        while True:
            message: str = self.stm_link.recv().strip()
            if message == "SNAP":
                self.logger.debug("SNAP command received from STM32.")
                results = self.snap_and_rec()
                self.stm_link.send(results)
...
#                     time.sleep(0.1)
    ...
    def snap_and_rec(self) -> None:
        ...
        draw_url = f"http://{API_IP}:6000/draw_bounding_boxes"
        filename = f"{int(time.time())}_task2.jpg"
        retry_count = 0
        self.last_class_name = None
            ...
            while retry_count < 3:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.info("[snap_and_rec] Image processing complete, sending data to STM")
                        results_stm = self.handle_image_results(results)
                        #results["obstacle_id"] = obstacle_id_with_signal
                        #self.android_queue.put(AndroidMessage("image-rec", results))
                        draw_response = requests.post(draw_url, json=results, timeout=10)

                        if draw_response.status_code == 200:
                            self.logger.info("[snap_and_rec] Bounding boxes drawn successfully.")
                        else:
                            self.logger.error(f"[snap_and_rec] Error drawing bounding boxes: {draw_response.text}")

                        return results_stm # ✅ Return after success
    ...
    def handle_image_results(self, result):
        if result["class_name"] == "Left Arrow":
            self.send_to_stm("LEFTaaa")
        elif result["class_name"]  == "Right Arrow":
            self.send_to_stm("RIGHTaaa")
